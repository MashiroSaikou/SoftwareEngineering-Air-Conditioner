 ...
 * @param fr_L  一阶费率
 */
ServeObject::init(int m, int t_hL, int t_lL, int d_tT,
...
ServeObject::setReady(bool isReady) {
 ...
 * @return
 */
bool ServeObject::dispatchRoom(int newRID, int oldnewRID, QJsonDocument r, int serveTime)
{
    ...
    if (oldRID >= 0 && oldnewRID < ROOMCOUNT)
        servedMap[oldnewRID] = false;
    else
        qDebug()<<"wrong oldRID"<<endl;

    rooms[oldnewRID].serveTime += serveTime;
    do_request(newRID, doc);
    return true;
}
 ...
 * @param s
 */
void ServeObject::get_fee_and_feerate(int rid, state_info *s) {
    // to do
}
